  ...
  class Inspector {
    ...
    getData(e) {
      ...
      if (e.target.id !== this.contentNode) {
        ...
        this.XPath = XPath;
        this.xpathlist.push('"'+XPath+'"');
        var output = "select "+this.xpathlist.join(",")+" from jshtml where url in (\""+this.win.location.href+"\") and selector=xpath"
        const contentNode = document.getElementById(this.contentNode);
        if (contentNode) {
          contentNode.innerText = output;
        } else {
          const contentHtml = document.createElement('div');
          contentHtml.innerText = output;
